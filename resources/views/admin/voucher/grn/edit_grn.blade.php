      ...
      <br>
        ...
        <div class="main-body ">
          <div class="page-wrapper m-t-0 m-l-1  p-10">
            ...
            <form id="edit_grn_id"  method="POST">
                @csrf
                {{ method_field("PUT") }}
            <div class="page-body">
                  <div class="row">
                        ...
                        <div class="row " style="margin-top: 5px;">
                            ...
                            <div class="col-sm-8 m-0 p-0"  style="margin-left: 5px;!important;" >
                               ...
                               <select  style="border-radius: 15px;" name="debit_ledger_id"  class="form-control m-1 js-example-basic-single  js-example-basic debit_ledger_id" required>
                                ...
                                <input type="text" name="credit_ledger_name" id="credit_ledger_name" class="form-control credit_ledger_name" value="{{$credit->ledger_name}}" style="margin-bottom: 4px !important;margin-top: 2px;"/>
                                <span id='error_inventory_value_affected' class="text-danger"></span>
                                <input type="hidden" name="credit_ledger_id" value="{{$credit->ledger_head_id}}" id="credit_ledger_id" class="form-control credit_ledger_id"  style="margin-bottom: 4px !important;border-radius: 15px;margin-top: 2px;"/>
                            </div>
                      </div>
                    </div>
                    <div class="col-sm-4" >
            ...
            <div align="center">
              <button  type="submit" class="btn btn-info edit_grn_btn cansale_btn" style="width:120px;border-radius: 15px;"><span class="m-1 m-t-1" style="color:#404040"><i class="fa fa-save" style="font-size:18px;" ></i></span><span >Update</span></button>
...
// credit ledger searching
$(document).ready(function () {
  $('.credit_ledger_name').autocomplete({
      // minChars: 1,
...
$(document).ready(function(){
  ...
  // get stock item
    $.ajax({
            type: 'GET',
            url: "{{ url('edit/purchase/stock/in')}}",
            async: false,
            data: {
                tran_id:t_m_id
            },
            ...
            $.each(response, function(i,val) {
                ...
                <td  class="m-0 p-0">
                    <input class="form-control rate text-right"  name="rate[]" data-field-name="rate" type="number" step="any" data-type="rate" id="rate_${i}" value="${Math.abs(val.rate).toFixed(amount_decimals)}" for="${i}" ${stock_item_price_typeabe==0?'readonly':''} />
                </td>
                <td  class="m-0 p-0">
                ...
                <td  class="m-0 p-0">
                    <input class="form-control amount  text-right" type="number" step="any"  name="amount[]" id="amount_${i}"  value="${Math.abs(val.total).toFixed(amount_decimals)}" ${amount_typeabe==1?'readonly':''}  for="${i}"/>
                </td>
    ...
    $(document).on('click', '.btn_remove', function() {
        ...
        arr.push($(this).closest('tr').find('.stock_in_id').val());
        $('.delete_stock_in_id').val(arr);
  ...
  function calculation_total(){
      ...
      $('.total_credit').val(parseFloat(amount).toFixed(amount_decimals));
  ...
  $('#commission_append').on('keyup change', '.commission_amount,.commision_cal', function() {
       ...
       let current_row=$(this).closest('tr').attr('id');
        if(commision_cal==1){
            $('#commission_append tr').find('#get_commission_'+current_row).val(parseFloat((commission_amount/100)*parseFloat(total_credit)).toFixed(amount_decimals));
        }
 ...
 function commission_per_calcucation(){
    ...
    $('#commission_append tr').each(function(i){
        ...
        if(commision_cal==1 || commision_cal==2){
            $(this).find('.get_commission').val(parseFloat((commission_amount/100)*parseFloat(total_credit)).toFixed(amount_decimals));
        }
        ...
        $.each(JSON.parse(debit_credit_value.replace(/&quot;/g,'"')), function(i,val) {
            if(val.commission_type){
                $('#commission_append').append(`<tr id="${i}">
                    ...
                    <td  colspan="2" class="m-0 p-0" > <select name="commission_ledger_id[]" class="form-control select2  commission_ledger_id${val.debit_credit_id} commission_select2_${i}"  id="select_${i}" required>
    ...
    $('#add_commission').click(function() {
        ComRowCount++;
            $('#commission_append').append(`<tr id="${ComRowCount}">
                    ...
                    <td class="text-right ">Commission: </td>
                    <td  colspan="2" class="m-0 p-0" > <select name="commission_ledger_id[]" class="form-control select2 commission_select2_${ComRowCount}" id="select_${ComRowCount}" required>
                    <option value="0">--Select--</option>
                        {!!html_entity_decode($ledger_commission_tree)!!}
                    </select></td>
                    <td class="m-0 p-0">
            ...
            if(selectedData && selectedData.item && selectedData.item.data){
                ...
                data = selectedData.item.data;
                        if(data.godown_id){
                            ...
                            $(".cansale_btn").attr("disabled", false);
                        }
                        if(data.stock_item_id){
                            $('#product_id_'+rowNo).val(data.stock_item_id);
                            item_check.push(data.stock_item_id);
                            $('#per_'+rowNo).val(data.symbol);
                            $('#measure_id_'+rowNo).val(data.unit_of_measure_id);
                            $(this).closest('tr').find('.qty').attr('readonly', false);
                            $(this).closest('tr').find('.rate').attr('readonly', false);
...
<script>
$(document).ready(function(){
    $("#edit_grn_id").submit(function(e) {
        e.preventDefault();
        const fd = new FormData(this);
        var id="{{$data->tran_id}}";
        $(".edit_grn_btn").text('Upadte');
        $.ajax({
                url: "{{ url('voucher-grn') }}" + '/' + id,
                method: 'post',
                data: fd,
                cache: false,
                contentType: false,
                ...
                error : function(data,status,xhr){
                    if(data.status==404){
                        ...
                        data.responseJSON.data.invoice_no[0];
                    }
                }
        });
    });
