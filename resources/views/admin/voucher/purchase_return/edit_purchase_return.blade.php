...
@section('title','Voucher Purchase Return')
@push('css')
      ...
      <br>
        ...
        <div class="main-body ">
          <div class="page-wrapper m-t-0 m-l-1  p-10">
            ...
            <form id="edit_purchase_return_id"  method="POST">
                @csrf
                {{ method_field("PUT") }}
            <div class="page-body">
                  <div class="row">
                    ...
                    <div class="col-sm-4" >
                        <div class="row " style="margin-top: 5px;">
                            ...
                            <div class="col-sm-8 m-0 p-0"  style="margin-left: 5px;!important;" >
                                <input type="hidden" name="credit_id" value="{{$credit->debit_credit_id??0}}" class="form-control credit_id" />
                               <select  style="border-radius: 15px;" name="credit_ledger_id"  class="form-control m-1 js-example-basic-single  js-example-basic credit_ledger_id" required>
                                ...
                                <label id="debit_amont" style="font-weight: bold;font-size: 18px!important; margin:2px;" >{{$credit_sum_value??'0.000'}}</label>
                                ...
                                <input type="hidden" name="debit_ledger_id" value="{{$debit->ledger_head_id??0}}" id="debit_ledger_id" class="form-control debit_ledger_id"  style="margin-bottom: 4px !important;border-radius: 15px;margin-top: 2px;"/>
                            </div>
                      </div>
                    </div>
                    <div class="col-sm-4" >
                ...
                @if (company()->customer_id!=0)
                    <div class="form-group"  >
                        <label  for="exampleInputEmail1">Customer :</label>
                            <select name="customer_id" class="form-control js-example-basic-single p_customer_id " required>
            ...
            <div align="center">
              <button  type="submit" class="btn btn-info edit_purchase_return_btn" style="width:120px;border-radius: 15px;"><span class="m-1 m-t-1" style="color:#404040"><i class="fa fa-save" style="font-size:18px;" ></i></span><span>Update</span></button>
...
$('.credit_ledger_id').val('{{$credit->ledger_head_id}}');
      ...
      select: function (event, ui) {
        $.ajax({
                ...
                success: function(response){
                    ...
                    $('#debit_amont').text(response.data);
                    $("#debit_ledger_name").val(ui.item.value);
                    $("#debit_ledger_id").val(ui.item.ledger_head_id);
  ...
  let p;
    ...
    $.ajax({
            ...
            $.each(response.data, function(i,val) {
                ...
                <td  class="m-0 p-0">
                      <input class="form-control stock text-right"   data-field-name="stock" type="number" class="stock" value="${(parseInt((val.stock_in_sum)-(val.stock_out_sum))).toFixed(amount_decimals)}" id="stock_${i}"  for="${i}" readonly />
                </td>
                <td  class="m-0 p-0">
    ...
    $.each(JSON.parse(debit_credit_value.replace(/&quot;/g,'"')), function(i,val) {
        if(val.commission_type){
            $('#commission_append').append(`<tr id="${i}">
                ...
                <td  colspan="3" class="m-0 p-0" > <select name="commission_ledger_id[]" class="form-control select2  commission_ledger_id${val.debit_credit_id} commission_select2_${i}"  id="select2_${i}" required>
   ...
   $('#add_commission').click(function() {
      ComRowCount++;
        $('#commission_append').append(`<tr id="${ComRowCount}">
                ...
                <td class="text-right ">Commission: </td>
                <td  colspan="3" class="m-0 p-0" > <select name="commission_ledger_id[]" class="form-control select2 commission_select2_${ComRowCount}" id="select2_${ComRowCount}" required>
                <option value="0">--Select--</option>
                    {!!html_entity_decode($ledger_commission_tree)!!}
                </select></td>
                <td class="m-0 p-0">
...
<script>
$(document).ready(function(){
$("#edit_purchase_return_id").submit(function(e) {
  e.preventDefault();
  const fd = new FormData(this);
  var id="{{$data->tran_id}}";
  $(".edit_purchase_return_btn").text('Update');
  $.ajax({
          url: "{{ url('voucher-purchase-return') }}" + '/' + id,
          method: 'post',
          data: fd,
          cache: false,
          contentType: false,
