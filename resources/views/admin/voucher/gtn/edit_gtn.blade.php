      ...
      <br>
        ...
        <div class="main-body ">
          <div class="page-wrapper m-t-0 m-l-1  p-10">
            ...
            <form id="edit_gtn_id"  method="POST">
                @csrf
                {{ method_field("PUT") }}
            <div class="page-body">
                  <div class="row">
            ...
            <div align="center">
              <button  type="submit" class="btn btn-info add_received_btn" style="width:120px;border-radius: 15px;"><span class="m-1 m-t-1" style="color:#404040"><i class="fa fa-save" style="font-size:18px;" ></i></span><span>Update</span></button>
            ...
            success: function (response) {
             $.each(response.data, function(i,val) {
                 $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${i}">
                  ...
                  <td  class="m-0 p-0">
                      <input class="form-control rate text-right"  name="rate[]" data-field-name="rate" type="number" data-type="rate" id="rate_${i}" value="${val.rate.toFixed(amount_decimals)}" for="${i}" ${stock_item_price_typeabe==0?'readonly':''} />
                  </td>
                  <td  class="m-0 p-0">
                  ...
                  <td  class="m-0 p-0">
                      <input class="form-control amount  text-right" type="number"  name="amount[]" id="amount_${i}"  value="${val.total.toFixed(amount_decimals)}" ${amount_typeabe==1?'readonly':''}  for="${i}"/>
                  </td>
...
<script>
$(document).ready(function(){
$("#edit_gtn_id").submit(function(e) {
  e.preventDefault();
  const fd = new FormData(this);
  var id="{{$data->tran_id}}";
  $("#edit_gtn_btn").text('Update');
  $.ajax({
          url: "{{url('voucher-gtn')}}" + '/' + id,
          method: 'post',
          data: fd,
          cache: false,
          contentType: false,
