      ...
      <br>
        ...
        <div class="main-body ">
          <div class="page-wrapper m-t-0 m-l-1  p-10">
            ...
            <form id="edit_sales_id"  method="POST">
                @csrf
                {{ method_field("PUT") }}
            <div class="page-body">
                  <div class="row">
                    ...
                    <div class="col-sm-4" >
                        <div class="row " style="margin-top: 5px;">
                            ...
                            @if($voucher->commission_is!=1)
                                <div class="col-sm-8 m-0 p-0"  style="margin-left: 5px;!important;" >
                                    <input type="hidden" name="debit_id" value="{{$credit->debit_credit_id??0}}" class="form-control debit_id" />
                                    ...
                                    <input type="hidden" name="credit_id" value="{{$debit->debit_credit_id}}" class="form-control debit_id" />
                                    <label id="credit_amont" style="font-weight: bold;font-size: 18px!important; margin:2px;" >{{$credit_sum_value??'0.000'}}</label>
                                    <input type="text" name="debit_ledger_name" id="debit_ledger_name" class="form-control debit_ledger_name" value="{{$debit->ledger_name??0}}" style="margin-bottom: 4px !important;margin-top: 2px;"/>
                                    <span id='error_inventory_value_affected' class="text-danger"></span>
                                    <input type="hidden" name="credit_ledger_id" value="{{$debit->ledger_head_id??0}}" id="credit_ledger_id" class="form-control credit_ledger_id"  style="margin-bottom: 4px !important;border-radius: 15px;margin-top: 2px;"/>
                                </div>
                            @else
                                <div class="col-sm-8 m-0 p-0"  style="margin-left: 5px;!important;" >
                                    ...
                                    <input type="hidden" name="credit_id" value="{{$debit->debit_credit_id??0}}" class="form-control debit_id" />
                                    <label id="credit_amont" style="font-weight: bold;font-size: 18px!important; margin:2px;" >{{$credit_sum_value??'0.000'}}</label>
                                    <input type="text" name="debit_ledger_name" id="debit_ledger_name" class="form-control debit_ledger_name" value="{{$debit->ledger_name??''}}" style="margin-bottom: 4px !important;margin-top: 2px;"/>
                                    <span id='error_inventory_value_affected' class="text-danger"></span>
                                    <input type="hidden" name="credit_ledger_id" value="{{$debit->ledger_head_id??0}}" id="credit_ledger_id" class="form-control credit_ledger_id"  style="margin-bottom: 4px !important;border-radius: 15px;margin-top: 2px;"/>
                                </div>
                          @endif
                            ...
                            <div class="col-sm-8 m-0 p-0"  style="margin-left: 5px;!important;" >
                                ...
                                <select style="margin-top: 2px; border-radius: 15px;" name="unit_or_branch"  class="form-control m-1 js-example-basic-single  js-example-basic  unit_or_branch" required>
                                    @foreach ($branch_setup as $unit_branchs)
                                     <option {{$data->unit_or_branch?'selected':''}} value="{{ $unit_branchs->id }}">{{$unit_branchs->branch_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                      </div>
                ...
                <table class="table customers" style=" border: none !important; margin-top:5px;" >
                  <thead>
                    <tr >
                        ...
                        <th class="{{$voucher->commission_is==1?'col-2':'col-2'}}">Amount</th>
                        @if($voucher->commission_is==1)
                            <th class="col-1">Sales Ledger</th>
                            <th class="col-1">Commission</th>
                            <th class="col-1">  Type- %</th>
                            <th class="col-2"> Comm <br> Amount</th>
                        @endif
                        @if($voucher->godown_motive==3)
                  ...
                  <tfoot>
                      <tr>
                        ...
                        @elseif($voucher->godown_motive==2)
                          <td ></td>
                        @endif
                        <td><input type="text" style="font-weight: bold;"  class="total_qty form-control text-right" readonly></td>
                        <td></td>
                        <td></td>
                  ...
                  <tfoot>
                          <tr >
                              ...
                              <td></td>
                              <td></td>
                              <td><input type="number" style="font-weight: bold;" name="product_wise_get_commission_without"  class=" form-control text-right product_wise_get_commission_without" readonly></td>
                              <td class="col-1 {{$voucher->remark_is==0?'d-none':'' }}"></td>
                          </tr>
                  </tfoot>
                  <tfoot>
            ...
            <div align="center">
              <button  type="submit" class="btn btn-info edit_sales_btn" style="width:120px;border-radius: 15px;"><span class="m-1 m-t-1" style="color:#404040"><i class="fa fa-save" style="font-size:18px;" ></i></span><span >Update</span></button>
...
if(commission_is!=1){
    $('.debit_ledger_id').val('{{$credit->ledger_head_id}}');
}
...
$(document).ready(function(){
 // voucher setup and setting variable
  ...
  // get item
    $.ajax({
            ...
            success: function (response) {
                $.each(response.data, function(i,val) {
                    ...
                    ${(commission_is==1)?
                    `<td  class="m-0 p-0">
                        <input class="com_credit" type="hidden" name="debit_id[]" value="${val.credit_id}">
                        <select   name="debit_ledger_id[]"  class="form-control debit_ledger_id${val.credit_id}" required>
                        <option value="0">--Select--</option>
                        {!!html_entity_decode($ledger_tree)!!}
                        </select>
                    </td>
                    <td  class="m-0 p-0">
                        ...
                        <input class="commission_id" type="hidden" name="commission_id[]" value="${val.commission_id}">
                        <input class="form-control ledger_name  autocomplete_txt"  name="ledger_name[]" data-field-name="ledger_name"  type="text" data-type="ledger_name" id="ledger_name_${i}" value="${val.commission_ledger_name?val.commission_ledger_name:''}" autocomplete="off" for="${i}"  />
                        <input class="form-control product_wise_commission_ledger " name="product_wise_commission_ledger[]" data-field-name="product_wise_commission_ledger"  type="hidden" data-type="product_wise_commission_ledger" id="product_wise_commission_ledger_${i}" value="${val.commission_ledger_id?val.commission_ledger_id:''}"  autocomplete="off" for="${i}"/>
                    </td>
                    <td  class="m-0 p-0">
                        <div style="display: flex;flex-direction: row;">
                                <select name="product_wise_commission_cal[]" class="form-control left-data product_wise_commision_cal"  style="margin:0%;" >
                                    <option ${val.commission_commission_type==1?'selected':''} value="1">(-) %</option>
                                    <option ${val.commission_commission_type==2?'selected':''} value="2">(+) %</option>
                                    <option ${val.commission_commission_type==3?'selected':''} value="3">(-)</option>
                                    <option ${val.commission_commission_type==4?'selected':''} value="4">(+)</option>
                                </select>
                                <input type="text" name="product_wise_commission_amount[]" id="product_wise_commission_amount_${i}" value="${val.commission_commission?val.commission_commission:''}" class="product_wise_commission_amount form-control text-right" >
                        </div>
                    </td>
                    <td class="m-0 p-0"><input type="number" step="any" name="product_wise_get_commission[]"  id="product_wise_get_commission_${i}" value="${val.commission_debit?val.commission_debit:(val.commission_credit?val.commission_credit:'')}" class="product_wise_get_commission form-control text-right" ></td>
                    `:`""`}
                    ...
                    $(`.debit_ledger_id${val.credit_id}`).val(val.credit_ledger_id);
                    p=i;
                });
    ...
    //remove row
    var arr = [];
    let com_debit_credit_arr=[];
    $(document).on('click', '.btn_remove', function() {
        var button_id = $(this).attr('id');
        $('#row'+button_id+'').remove();
        if($(this).closest('tr').find('.com_credit').val()){
            com_debit_credit_arr.push($(this).closest('tr').find('.com_credit').val());
            com_debit_credit_arr.push($(this).closest('tr').find('.commission_id').val());
           $('.delete_debit_credit_id').val(com_debit_credit_arr);
        }
        ...
        for(var row=1; row<6;row++) {
            rowCount++;
                $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${rowCount}">
                    ...
                    <td  class="m-0 p-0">
                        <input type="hidden" name="commission_is" value="${commission_is}">
                        <input class="form-control ledger_name  autocomplete_txt"  name="ledger_name[]" data-field-name="ledger_name"  type="text" data-type="ledger_name" id="ledger_name_${rowCount}" value="{{$ledger_id_wise->ledger_name??''}}" autocomplete="off" for="${rowCount}"  />
                        <input class="form-control product_wise_commission_ledger " name="product_wise_commission_ledger[]" data-field-name="product_wise_commission_ledger"  type="hidden" data-type="product_wise_commission_ledger" id="product_wise_commission_ledger_${rowCount}" value="{{$ledger_id_wise->ledger_head_id??''}}" autocomplete="off" for="${rowCount}"/>
                    </td>
                    <td  class="m-0 p-0">
                        <div style="display: flex;flex-direction: row;">
                                <select name="product_wise_commission_cal[]" class="form-control left-data product_wise_commision_cal"  style="margin:0%;" >
 ...
 function commission_per_calcucation(){
   ...
   function  product_wise_commission_per_calcucation(){
            $('#orders tr').each(function(i){
                let commission_amount=$(this).find('.product_wise_commission_amount').val();
                let get_commission = $(this).find('.product_wise_get_commission ').val();
                let commision_cal=$(this).find('.product_wise_commision_cal').val();
                let total_amount =$(this).find('.amount').val();
                if(commision_cal==1 || commision_cal==2){
 ...
 // get commission calcucation
  ...
  //edit value commission
    $.each(JSON.parse(debit_credit_value.replace(/&quot;/g,'"')), function(i,val) {
        if(val.commission_type&&val.comm_level==1){
            $('#commission_append').append(`<tr id="${i}">
                ...
                <td  colspan="3" class="m-0 p-0" > <select name="commission_ledger_id[]" class="form-control select2  commission_ledger_id${val.debit_credit_id} commission_select2"  id="select_${i}" required>
   ...
   $('#add_commission').click(function() {
      ComRowCount++;
        $('#commission_append').append(`<tr id="${ComRowCount}">
                ...
                <td class="text-right ">Commission: </td>
                <td  colspan="3" class="m-0 p-0" > <select name="commission_ledger_id[]" class="form-control select2 commission_select2" id="select_${ComRowCount}" required>
                <option value="0">--Select--</option>
                    {!!html_entity_decode($ledger_commission_tree)!!}
                </select></td>
                <td class="m-0 p-0">
...
// update sales
$(document).ready(function(){
$("#edit_sales_id").submit(function(e) {
  e.preventDefault();
    const fd = new FormData(this);
    var id="{{$data->tran_id}}";
    $("#edit_sales_btn").text('Add');
    $.ajax({
            url: "{{ url('voucher-sales') }}" + '/' + id,
            method: 'post',
            data: fd,
            cache: false,
            contentType: false,
