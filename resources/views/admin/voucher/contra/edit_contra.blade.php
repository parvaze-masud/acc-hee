      ...
      <br>
        ...
        <div class="main-body ">
          <div class="page-wrapper m-t-0 m-l-1  p-10">
            ...
            <form id="edit_contra_id"  method="POST">
                @csrf
                {{ method_field("PUT") }}
                <div class="page-body">
                  <div class="row">
                  ...
                  <div align="center">
                    <button  type="submit" class="btn btn-info add_contra_btn" style="width:120px;border-radius: 15px;"><span class="m-1 m-t-1" style="color:#404040"><i class="fa fa-save" style="font-size:18px;" ></i></span><span >Update</span></button>
                        ...
                        <td  class="m-0 p-0">
                            <input class="form-control  blance  text-right " name="blance[]"  data-field-name="blance" type="text" class="blance" id="blance_${i}"
                             value="${(val.nature_group==1||val.nature_group==3)
                                ...
                                ?(parseFloat(val.credit_sum_2-val.debit_sum_2).toFixed(amount_decimals) +' Cr'):'')}" for="${i}" readonly/>
...
<script>
$(document).ready(function(){
   $("#edit_contra_id").submit(function(e) {
        e.preventDefault();
        const fd = new FormData(this);
        var id=$('.tran_id').val();
        $("#edit_contra_btn").text('Upadte');
        $.ajax({
            url: "{{ url('voucher-contra') }}" + '/' + id,
                method: 'post',
                data: fd,
                cache: false,
                contentType: false,
