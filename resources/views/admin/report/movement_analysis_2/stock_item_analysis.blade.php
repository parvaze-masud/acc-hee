...
function get_stock_item_analysis(response){
              total_inwards_qty=0;total_inwards_value=0; total_outwards_qty=0; total_outwards_value=0;
                ...
                if(response.purchase){
                    ...
                    $.each(response.purchase, function(key, v) {
                           total_inwards_qty+=(v.stock_in_qty||0);total_inwards_value+=(v.stock_in_total||0);
                            html+=` <tr id='${v.ledger_head_id}' class="left left-data editIcon table-row">
                ...
                html+=` <tr><td style="font-weight: bolder;font-size: 18px;" colspan="2">Total :</td><td style=";font-size: 16px;font-weight: bolder;">${total_inwards_qty.toFixed(amount_decimals).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</td><td style=";font-size: 16px;font-weight: bolder;">${(Math.abs((total_inwards_value)/(total_inwards_qty))||0).toFixed(amount_decimals).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</td><td style=";font-size: 16px;font-weight: bolder;">${total_inwards_value.toFixed(amount_decimals).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</td></tr> `;
               ...
               html+=` <tr><td style="font-weight: bolder;font-size: 18px;" colspan="2">Total :</td><td style=";font-size: 16px;font-weight: bolder;">${total_outwards_qty.toFixed(amount_decimals).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</td><td style=";font-size: 16px;font-weight: bolder;">${(Math.abs((total_outwards_value)/(total_outwards_qty))||0).toFixed(amount_decimals).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</td><td style=";font-size: 16px;font-weight: bolder;">${total_outwards_value.toFixed(amount_decimals).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</td></tr> `;
...
$('.sd').on('click','.table-row',function(e){
        e.preventDefault();
        let  ledger_head_id=$(this).closest('tr').attr('id');
        let stock_item_id=$(".stock_item_id").val();
        let godown_id=$('.godown_id').val();
        let form_date=$('.from_date').val();
        let to_date=$('.to_date').val();
        let purchase_in=$('.purchase').is(':checked')?$('.purchase').val():0;
        let grn_in=$('.grn').is(':checked')?$('.grn').val():0;
        let purchase_return_in=$('.purchase_return').is(':checked')?$('.purchase_return').val():0;
        let journal_in=$('.journal').is(':checked')?$('.journal').val():0;
        let stock_journal_in=$('.stock_journal').is(':checked')?$('.stock_journal').val():0;
        let sales_return_out=$('.sales_return').is(':checked')?$('.sales_return').val():0;
        let gtn_out=$('.gtn').is(':checked')?$('.gtn').val():0;
        let sales_out=$('.sales').is(':checked')?$('.sales').val():0;
        let journal_out=$('.journal').is(':checked')?$('.journal').val():0;
        let stock_journal_out=$('.stock_journal').is(':checked')?$('.stock_journal').val():0;
