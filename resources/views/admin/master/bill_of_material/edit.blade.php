    ...
    <div class="pcoded-inner-content  ">
        ...
        <div class="main-body ">
            <div class="page-wrapper m-t-0 m-l-1  p-10">
                    ...
                    <div class="row align-items-end" style="margin-bottom: 0%px !important;">
                        <div class="col-lg-8 ">
                            <div class="page-header-title m p-0" style="margin-bottom:7px !important;">
                                <div class="d-inline ">
                                    <h4 style="color: green;font-weight: bold;"> [Bill Of Material Update]</h4>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4 ">
                            <div style="float: right; margin-left: 5px;">
                                <a style=" float:righttext-decoration: none;" href="{{route('master-dashboard')}}"><span class="fa fa-info-circle m-1"style="font-size:27px; color:#00b8e6;"></span><span style="float:right;margin:2px; padding-top:5px; color: color: white;#">Help</span></a>
                            </div>
                            ...
                            <div style="float: right; margin-left:9px">
                                <a style=" float: righttext-decoration: none; "  href="{{route('bill-of-material.index')}}"><span class="fa fa-eye m-1"  style="font-size:27px; color:#00b8e6;"></span><span style="float:right;margin:2px; padding-top:5px; ">View</span></a>
                ...
                <form id="edit_bill_of_material_id" method="POST">
                    @csrf
                    {{ method_field("PUT") }}
                    <div class="page-body">
                        <div class="row">
                            <div class="col-sm-6">
                                ...
                                <input class="form-control " type="text" name="bom_name" value="{{$data->bom_name}}">
                                <span id='error_bill_of_material' class="text-danger"></span>
                                <input class="bom_id" type="hidden"  name="bom_id" value="{{$data->bom_id}}" >
                    ...
                    <fieldset style="border:1px rgb(32, 30, 30) solid;">
                        <legend style="width: 32%;  font-size: 18px;">Raw Materials / Source / Consumption / Stock Out
                        </legend>
                        <div class="row">
                    ...
                    <fieldset style="border:1px rgb(32, 30, 30) solid;">
                        <legend style="width: 33%;font-size: 18px;">Finished Goods / Destination / Production / Stock In
                        </legend>
                        <div class="row">
                            <div class="table-responsive">
                                <table class="table customers" style=" border: none !important; margin-top:5px;">
                    ...
                    <div align="center" class="m-1">
                        <button type="submit" class="btn btn-info bill_of_material_btn"style="width:120px;border-radius: 15px;"><span class="m-1 m-t-1" style="color:#404040"><i class="fa fa-save" style="font-size:18px;"></i></span><span>Update</span></button>
                        @if(user_privileges_check('master','Material','delete_role'))
...
$(document).ready(function() {
    ...
    var bill_of_material_details="{{$bill_of_material_details}}";
    let p_in,p_out;
  // source
        $.each(JSON.parse(bill_of_material_details.replace(/&quot;/g,'"')), function(i,val) {
          if(val.details_copy==1){
            $('#orders_in').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row_in${i}">
                <input class="form-control  stock_in_id m-0 p-0"  name="stock_in_id[]" type="hidden" data-type="stock_in_id" value="${val.details_id}" id="stock_in_id_${i}"  for="${i}"/>
                ...
                <td  class="m-0 p-0">
                    <input class="form-control  autocomplete_txt" name="product_in_name[]" data-field-name="product_name"  type="text" data-type="product_in_name" id="product_in_name_${i}" value="${val.product_name}" autocomplete="off" for="${i}"  />
                </td>
                <td  class="m-0 p-0">
                      <input class="form-control  unit_in" name="unit_in[]"  data-field-name="unit_in" type="text"  id="unit_in_${i}"  for="${i}" value="${val.symbol}" />
                </td>
                <td  class="m-0 p-0">
                    <input class="form-control qty_in text-right " name="qty_in[]"  data-field-name="qty_in" type="number"  value="${val.qty}" class="qty_in" id="qty_in_${i}"  for="${i}"  />
                </td>
            </tr>`);
            p_in=i;
          }
        ...
        // destination
         $.each(JSON.parse(bill_of_material_details.replace(/&quot;/g,'"')), function(i,val) {
                if(val.details_copy==2){
                    $('#orders_out').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row_out${i}">
                        <input class="form-control  stock_out_id m-0 p-0"  name="stock_out_id[]" type="hidden" data-type="stock_out_id" value="${val.details_id}" id="stock_in_id_${i}"  for="${i}"/>
                        ...
                        <td  class="m-0 p-0">
                        <input class="form-control  autocomplete_txt" name="product_out_name[]" data-field-name="product_name"  type="text" data-type="product_out_name" id="product_out_name_${i}" value="${val.product_name}" autocomplete="off" for="${i}"  />
                        </td>
                        <td  class="m-0 p-0">
                           <input class="form-control  out_unit" name="out_unit[]"  data-field-name="out_unit" type="text"  id="out_unit_${i}"  for="${i}"  value="${val.symbol}" />
                        </td>
                        <td  class="m-0 p-0">
                            <input class="form-control qty_out text-right " name="qty_out[]"  data-field-name="qty_out" type="number" class="qty_out" id="qty_out_${i}" value="${val.qty}" for="${i}"  />
                        </td>
                    </tr>`);
                    p_out=i;
                }
   ...
   //source  remove
    ...
    function button_total_cal_in() {
        ...
        $('#orders_in tr').each(function(i) {
            if (parseFloat($(this).find('.qty_in').val())) qty_in += parseFloat($(this).find('.qty_in')
                .val());
        })
    ...
    function button_total_cal_out() {
        ...
        $('#orders_out tr').each(function(i) {
            if (parseFloat($(this).find('.qty_out').val())) qty_out += parseFloat($(this).find(
                '.qty_out').val());
        })
...
$(document).ready(function() {
    var id=$('.bom_id').val();
    $("#edit_bill_of_material_id").submit(function(e) {
        e.preventDefault();
        const fd = new FormData(this);
        $("#edit_bill_of_material_btn").text('Add');
        $.ajax({
            url: "{{ url('bill-of-material') }}" + '/' + id,
            method: 'post',
            data: fd,
            cache: false,
            contentType: false,
            ...
            if (result.value) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        window.location = "{{url('bill-of-material')}}"
                        swal_message(data.message,'success','Successfully');
                    },
                    error: function () {
                        swal_message(data.message,'error','Error');
